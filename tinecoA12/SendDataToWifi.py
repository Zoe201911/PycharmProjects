...
ser1 = serial.Serial('/dev/cu.SLAB_USBtoUART', '115200', timeout=30)
...
def factoryReset():
    ...
    print(str1)
    str2 = ''.join(['%02X' %b for b in str1]).lower()
    print("主机发送的指令是：",str2)
    ser1.write(str1)
    print("*************")
...
#获取WiFi设备信息，发射热点
def  connectWifiInfo():
    data = {"ret":"ok","sc":1,"ssidPrefix":"Tineco_","name":"0123456789012345a045","type":"q5ez2q"}
    str = '`FPGetDeviceCap' + '\0' + '\0' + 'pdcap' + '\0' + 'j' + json.dumps(data,ensure_ascii=False) + '\0' + '\n'
    str = str.encode("utf-8")
    # str3 = json.dumps(data,ensure_ascii=False)
    print(str)
    str2 =''.join('%02X ' %b for b in str).lower()
    while True:
